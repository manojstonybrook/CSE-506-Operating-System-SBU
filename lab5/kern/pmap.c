...
mem_init_mp(void)
{
 	...
 	for(i = 0; i < NCPU; i++)
	{
	   size = KSTKSIZE + KSTKGAP; 
           addr = KSTACKTOP - (i)*size;
	   boot_map_region(boot_pml4e, addr - KSTKSIZE, KSTKSIZE, PADDR(percpu_kstacks[i]), PTE_W|PTE_P);
	   //boot_map_region(boot_pml4e, addr - size, KSTKGAP, PADDR(percpu_kstacks[i]) + KSTKSIZE, PTE_W);
...
mmio_map_region(physaddr_t pa, size_t size)
{
	...
	if((uint8_t*)base + size_up > (uint8_t*)MMIOLIM)        
	...
	base = (uintptr_t)((uint8_t*)base+size_up);
