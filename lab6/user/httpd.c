...
send_data(struct http_request *req, int fd)
{
	...
	// At half buffersize it is giving faulting access error at larger size
	char buf[BUFFSIZE/8];
	int len;
	while((len = read(fd, buf, BUFFSIZE/8)) > 0)
	{
...
send_file(struct http_request *req)
{
	...
	/*if((r = fd_lookup(fd, &filed)) < 0)
           goto end;

	if(devfile.dev_stat(filed, &status) < 0)
	  goto end;*/
