		   ...
		   envid_t dstenvid, void *dstva, int perm)
{
	...
	if(page_insert(env_dst->env_pml4e, p, dstva,perm))
	{
	  page_free(p);
	  return -E_NO_MEM;
	}
...
//Transmit
static int
sys_e1000_transmit(char *data, uint32_t len)
{
  int r;
  if((r = transmit_packet(data, len)) < 0)
	return r;
...
syscall(uint64_t syscallno, uint64_t a1, uint64_t a2, uint64_t a3, uint64_t a4, uint64_t a5)
{
	...
	switch (syscallno) {
		...
		case SYS_e1000_transmit:
			return sys_e1000_transmit((char *)a1, (uint32_t)a2); 
		case SYS_e1000_receive:
			return sys_e1000_receive((char *)a1);
